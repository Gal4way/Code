...
public class frameListener implements GoBangconfig,MouseListener{
	...
	public Integer unionWeight(Integer a,Integer b ) {
	  ...
	  public void mouseClicked(java.awt.event.MouseEvent e) {
		  ...
		  int countx=(Px/size)*size+x;
		  int county=(Py/size)*size+y;
		  ...
		  if(gf.isAvail[Arrayi][Arrayj]!=0) {
			  ...
			  if(gf.ChooseType==0) {
				  if(gf.turn==1) {
					  //先获取要落的地方
					  g.drawImage(blackchess,countx-x, county-y, size, size,null);
					  //设置当前位置已经有棋子了,棋子为黑子
					  ...
					  g.drawImage(whitechess,countx-x, county-y, size, size,null);
					  //设置当前位置已经有棋子了，棋子为白子
					  gf.ChessPositonList.add(new ChessPosition(Arrayi,Arrayj));
